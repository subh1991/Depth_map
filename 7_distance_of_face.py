...
#	right_img_hist_eq = cv2.equalizeHist(right_img_gray)
	...
	faces = face_cascade.detectMultiScale(left_img_gray, 1.3, 2)
	for (x,y,w,h) in faces:
		left_img = cv2.rectangle(left_img,(x,y),(x+w,y+h),(255,0,0),2)
		x_ = x
		y_ = y
		w_ = w
		h_ = h
...
#	xyz[:,:,2] = depth_img
	...
	print np.mean(depth_img[y_:int(y_+0.7*h_), x_:int(x_+0.7*w_)])
